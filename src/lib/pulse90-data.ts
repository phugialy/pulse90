...
export async function getKnockoutFixtures(): Promise<Map<number, KnockoutFixtureDetail>> {
  ...
  const { data, error } = await supabase
    .from("fixtures")
    .select(
      "match_number, stage, starts_at, status, home_score, away_score, home_penalty_score, away_penalty_score, home_team_id, away_team_id, home_placeholder, away_placeholder, venues(name, host_city)",
    )
    .gte("match_number", 73)
  ...
  for (const row of rows) {
    ...
    map.set(row.match_number, {
      ...
      homeScore: row.home_score,
      awayScore: row.away_score,
      homePenalties: row.home_penalty_score ?? null,
      awayPenalties: row.away_penalty_score ?? null,
